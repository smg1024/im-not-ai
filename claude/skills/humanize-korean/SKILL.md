...
**에이전트 정의 위치:** `claude/agents/`에 12종 정의. Claude Code 탐색 경로:
1. 플러그인 설치 시 — `plugin.json`의 `agents` 경로를 통해 `claude/agents/`를 번들로 제공(전역).
2. 스크립트 설치 시 — `install.sh`가 `claude/agents/*.md`를 `~/.claude/agents/`에 심링크(전역).
