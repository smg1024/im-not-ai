...
- Claude: `/humanize-korean` · Codex: `$humanize-korean` + custom agents 12종
...
- Codex용 스킬은 **Fast(단일 호출) 모드**를 기본으로 하며, 별도로 설치되는 custom agents는 명시적 위임에 사용할 수 있습니다. 정밀 strict 5인 오케스트레이션은 Claude Code 전용.
...
본체가 이제 Codex CLI Skills와 custom agents를 **공식 지원**합니다. 리포 클론 후 한 줄이면 스킬은 `~/.agents/skills/`, agent 12종은 `~/.codex/agents/`에 연결됩니다:
...
Codex에서 `$humanize-korean`으로 Fast 모드를 발동합니다(또는 `/skills` 메뉴). 함께 설치되는 TOML agent들은 Claude agent와 같은 역할 지침을 제공해 이름을 지정한 위임에 쓸 수 있습니다. Claude의 정밀 strict 5인 오케스트레이션은 아직 Claude Code 전용입니다. (Codex Desktop용 별도 어댑터로는 community 포트 [`Squirbie/im-not-ai-codex`](https://github.com/Squirbie/im-not-ai-codex)도 있습니다.)
